...
switch id
    case 1
        ...
        % u = [21.4,23.8,25.5,27.9,31.1,32.8,33.9,34.6,34.7,30.6];
        % u = [21,24,26,26,26,26,26,26,26,26];
        u = 30*ones(1,10);
        if 1
            % without offset
    ...
    %%
        for k = 1:num_trial
            ...
            for ei = 1:num_step
                ...
                % x1_fr(k,ei+1) = all_gain(ei)/all_gain(1)*(x1_hfr(k,ei+1)-u(ei))+u(ei);
                % x2_fr(k,ei+1) = all_gain(ei)/all_gain(1)*(x2_hfr(k,ei+1)-u(ei))+u(ei);
                
                x1_fr(k,ei+1) = x1_hfr(k,ei+1);
                x2_fr(k,ei+1) = x2_hfr(k,ei+1);
